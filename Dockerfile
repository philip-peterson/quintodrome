...
FROM --platform=$BUILDPLATFORM public.ecr.aws/docker/library/golang:1.27-alpine AS build-alpine
COPY --from=xx / /
...
FROM --platform=$BUILDPLATFORM public.ecr.aws/docker/library/golang:1.27-trixie AS base
