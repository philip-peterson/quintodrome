...
FROM --platform=$BUILDPLATFORM docker.io/library/golang:1.27-alpine AS build-alpine
COPY --from=xx / /
...
FROM --platform=$BUILDPLATFORM docker.io/library/golang:1.27-trixie AS base
